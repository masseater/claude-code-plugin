...
※各Phaseの詳細計画書は `/break-down-phase` コマンドで生成してください
...
- 不明箇所の数（「**不明**」と記載した箇所）
- 次のアクション:
  - 不明箇所がある場合: `/clarify-spec [taskname]` コマンドを実行して明確化
  - Phase別計画書を生成する場合: `/break-down-phase [taskname]` コマンドを実行
  - その他の提案
